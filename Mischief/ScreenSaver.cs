...
namespace HPScreen
{
    public class ScreenSaver : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            string folderPath = Path.GetFullPath(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"..\\..\\..\\", $"Content{Path.DirectorySeparatorChar}Sprites{Path.DirectorySeparatorChar}"));
            ...
            if (Directory.Exists(folderPath))
            {
                ...
                {
                    filename = Path.GetFileName(file).Replace(".png", "");
                    Graphics.Current.SpritesByName.Add(filename, Content.Load<Texture2D>($"Sprites/{filename}"));
                }

                string[] jpgFiles = Directory.GetFiles(folderPath, "*.jpg");
                foreach (string file in jpgFiles)
                {
                    filename = Path.GetFileName(file).Replace(".jpg", "");
                    Graphics.Current.SpritesByName.Add(filename, Content.Load<Texture2D>($"Sprites/{filename}"));
                }
            }
            else
            {
                throw new Exception("Could not load the screen saver images in the designated file.");
            }
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            // Add your entity drawing logic here:
            //your_entity.Draw();
        ...
        protected void Setup()
        {
            ...
            this.roomname = bglist[Ran.Current.Next(0, bglist.Count - 1)];
        ...
        protected void DrawBackground()
        {
            ...
            Graphics.Current.SpriteB.Draw(
                Graphics.Current.SpritesByName[this.roomname],
